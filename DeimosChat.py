...
# --- ДОБАВЛЕНИЕ ПОРТА ДЛЯ RENDER ---
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.send_response(200)
        self.end_headers()
        self.wfile.write(b"DeimosChat Bot is running!")
...
# Запускаем мини-сервер в отдельном потоке, чтобы он не мешал long-polling боту
...
async def cmd_fight(message: Message):
    ...
    if roll <= win_chance:
        ...
        # --- ДОБАВЛЕННАЯ ПРОВЕРКА НА 0 БОТ МОНЕТ ---
        restored_text = ""
        if USER_DATA[user_id]["bot_coins"] <= 0:
            USER_DATA[user_id]["bot_coins"] = 5
            restored_text = "\n\n⚠️ **Ваш баланс обнулился! Система экстренной поддержки выдала вам 5 бот монет.**"
        # -------------------------------------------
        ...
        await message.answer(
            ...
            f"💸 Потеряно валюты ботов: -{bet}. Баланс: {USER_DATA[user_id]['bot_coins']}{restored_text}",
            parse_mode="Markdown"
        )
